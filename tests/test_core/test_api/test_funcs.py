...
async def test_read_one_or_none():
    ...
    await write_one.a(fake)
    db_fake = await read_one_or_none.a(Fake, filter_by_fake_ids)
    assert db_fake == fake

    await delete_one.a(fake)
    assert await read_one_or_none.a(Fake, filter_by_fake_ids) is None
...
async def test_read_one():
    ...
    await write_one.a(fake)
    db_fake = await read_one.a(Fake, filter_by_fake_ids)
    assert db_fake == fake

    await delete_one.a(fake)
    with pytest.raises(ValueError):
        await read_one.a(Fake, filter_by_fake_ids)
